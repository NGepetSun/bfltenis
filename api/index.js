...
function advance(s,roundIndex,matchIndex,w){const r=s.rounds[roundIndex],m=r.matches[matchIndex];m.winner=w;m.status="done";const loser=same(m.players[0],w)?m.players[1]:m.players[0];if(roundIndex===s.rounds.length-1){s.podium.first=w;s.podium.second=loser;return}const next=s.rounds[roundIndex+1].matches[Math.floor(matchIndex/2)];const slot=matchIndex%2;next.players[slot]=w;if(next.players[0]&&next.players[1]){next.status="live";next.liveYoutube=""};if(loser&&roundIndex===s.rounds.length-2){/* final loser handled above */}}
...
function addLivePlayerToDraw(s,player){
  ...
  // A live/done match is locked. Only waiting/bye slots may be re-drawn.
  ...
  for(const m of first.matches||[]){
    if(m.status!=="live"&&m.status!=="done"){
      ...
      m.players=[null,null];m.winner=null;m.status="waiting";m.liveYoutube="";
    }
  }
  pool.push(player);
  // Also include registered players that are not already locked or represented by a locked first-round match.
  for(const p of s.players||[])if(p&&pn(p)!=="TBD"&&!locked.has(pn(p))&&!pool.some(x=>pn(x)===pn(p)))pool.push(p);
  shuffle(pool);
  let pi=0;
  for(const m of first.matches||[]){
    if(m.status==="live"||m.status==="done")continue;
    m.players=[pool[pi++]||null,pool[pi++]||null];
    m.winner=null;
    m.status=m.players[0]&&m.players[1]?"live":m.players[0]?"bye":"waiting";
    m.liveYoutube="";
  }
  // If all existing first-round slots are locked, create a new waiting match for the new entrant(s).
  while(pi<pool.length){
    const id="r1m"+(first.matches.length+1);
    const a=pool[pi++],b=pool[pi++];
    first.matches.push({id,players:[a,b||null],winner:null,status:b?"live":"bye",liveYoutube:""});
  }
  // Auto-resolve newly created byes without touching locked matches.
  for(let i=0;i<first.matches.length;i++){
    const m=first.matches[i];
    if(m.status==="bye"&&m.players[0]){
      m.winner=m.players[0];m.status="done";
      const next=s.rounds[1]?.matches[Math.floor(i/2)];
      if(next&&!next.players[i%2])next.players[i%2]=m.winner;
      if(next&&next.players[0]&&next.players[1]&&next.status!=="done"){next.status="live";next.liveYoutube=""}
    }
  }
}
...
const playerName=b.name.trim();
if(s.players.some(p=>pn(p).toLowerCase()===playerName.toLowerCase()))return json(res,400,{error:"Nama sudah ada"});
const player={name:playerName,youtube:cleanYoutube(b.youtube)};
s.players.push(player);
if(s.status!=="setup") addLivePlayerToDraw(s,player);
await save(s);
return json(res,200,{ok:true,state:s});
}
if(action==="update-player"){
const i=Number(b.index);
if(!s.players[i])return json(res,404,{error:"Player tidak ditemukan"});
const old=s.players[i];
const nextName=String(b.name??pn(old)).trim();
if(!nextName)return json(res,400,{error:"Nama wajib"});
if(nextName.toLowerCase()!==pn(old).toLowerCase()&&s.players.some((p,idx)=>idx!==i&&pn(p).toLowerCase()===nextName.toLowerCase()))return json(res,400,{error:"Nama sudah ada"});
if(s.status!=="setup"&&nextName!==pn(old))return json(res,400,{error:"Nama player tidak bisa diubah setelah acara dimulai"});
s.players[i]={name:nextName,youtube:cleanYoutube(b.youtube)};
// Player POV remains available as a default for matches that have not received a match-specific POV.
await save(s);return json(res,200,{ok:true});
}
if(action==="start"){if(s.players.length<2)return json(res,400,{error:"Minimal 2 player"});if(s.status!=="setup")return json(res,400,{error:"Tournament sudah dimulai"});s=init();s.players=(await get()).players;s.rounds=build(s.players).rounds;s.status="live";for(let i=0;i<s.rounds[0].matches.length;i++){let m=s.rounds[0].matches[i];if(m.status==="bye"){const w=m.players.find(Boolean);m.winner=w;m.status="done";const next=s.rounds[1]?.matches[Math.floor(i/2)];if(next){next.players[i%2]=w;if(next.players[0]&&next.players[1]){next.status="live";next.liveYoutube=""}}}}await save(s);return json(res,200,{ok:true})}
if(action==="set-live"){const mid=String(b.matchId||"");let found=mid==="third"?s.thirdPlace:null;if(!found)for(const r of s.rounds)for(const m of r.matches)if(m.id===mid)found=m;if(!found)return json(res,404,{error:"Match tidak ditemukan"});
// Admin may attach/change POV while the event is already running. A waiting match with two players becomes live here.
if(found.status==="done")return json(res,400,{error:"Match sudah selesai"});
if(found.players?.[0]&&found.players?.[1]&&found.status!=="live")found.status="live";
found.liveYoutube=cleanYoutube(b.youtube);await save(s);return json(res,200,{ok:true,state:s})}
